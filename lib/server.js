...
class Server {
  ...
  async connectToVPN(serverIP) {
    ...
    try {
      ...
      await sshClient.exec('mkdir -p /var/run/xl2tpd');
      await sshClient.exec('touch /var/run/xl2tpd/l2tp-control');

      await sshClient.exec('service strongswan restart; sleep 2');
      await sshClient.exec('service xl2tpd restart; sleep 2');

      await sshClient.exec('ipsec up myvpn; sleep 2');
      await sshClient.exec('echo "c myvpn" > /var/run/xl2tpd/l2tp-control; sleep 2');

      const gateway = await sshClient.exec(`ip route | grep 'default via' | awk '{print $3}'`);
      const clients = await sshClient.exec(`netstat -tn 2>/dev/null | grep :${this.port} | awk '{print $5}' | cut -d: -f1`);
      const clientIP = clients.split('\n')[0];

      if (!(await sshClient.exec(`ip route | grep ${serverIP}`))) {
        await sshClient.exec(`route add ${serverIP} gw ${gateway}; sleep 2`);
      }
      if (!(await sshClient.exec(`ip route | grep ${clientIP}`))) {
        await sshClient.exec(`route add ${clientIP} gw ${gateway}; sleep 2`);
      }
      if (!(await sshClient.exec(`ip route | grep 'default dev ppp0'`))) {
        await sshClient.exec('route add default dev ppp0');
      }
    } finally {
      await sshClient.dispose();
    }
  }
  async disconnectFromVPN() {
    const sshClient = new NodeSSH();
    try {
      await sshClient.connect({
        host: this.host,
      ...
      await sshClient.exec('echo "d myvpn" > /var/run/xl2tpd/l2tp-control; sleep 2');
      await sshClient.exec('ipsec down myvpn; sleep 2');
    } finally {
      await sshClient.dispose();
    }
  }
  async fetchVPNClientIP() {
    const sshClient = new NodeSSH();
    try {
      await sshClient.connect({
        host: this.host,
      ...
      return await sshClient.exec(`ifconfig ppp0 | grep 'inet addr' | cut -d: -f2 | awk '{print $1}'`);
    } finally {
      await sshClient.dispose();
    }
  }
