...
function logTitle(message, offset = 0) {
  const symbol = useUnicodeSymbols() ? ' ☰ ' : ' ≡ ';
  console.log(''.padStart(offset) + chalk.bgHex('#3C9DFF').black(symbol + message + ' '));
}
function logSubtitle(message, offset = 0) {
  const symbol = useUnicodeSymbols() ? ' ─ ' : ' ─ ';
  console.log(''.padStart(offset) + chalk.bgHex('#80C6FF').black(symbol + message + ' '));
}
module.exports = { parseVersion, listVersion, findVersion, logSuccess, logWarning, logError, logTitle, logSubtitle };
